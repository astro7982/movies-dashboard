...
app.get('/movies', async (req, res) => {
  ...
  const movies = await getUrlContents('http://movies:3000/movies', require('node-fetch'))
  res.type('json')
...
app.get('/directors', async (req, res) => {
  ...
  const directors = await getUrlContents('http://directors:3001/directors', require('node-fetch'))
  res.type('json')
...
app.get('/actors', async (req, res) => {
  ...
  const actors = await getUrlContents('http://actors:3002/actors', require('node-fetch'))
  res.type('json')
  res.send(JSON.stringify({ dashboard: actors }))

})

app.listen(port, () => { console.log(`Listening at http://dashboard:${port}`)})
